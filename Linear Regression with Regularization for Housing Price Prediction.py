...
# same as root mean square error but with the new term lamda 
...
for j in range(13):
    #cross_validation with best alpha to get the best hypothesis order
...
print("J fn cost of y_test without regularization :", cost_Y_test)
print("J fn cost of y_test with regularization :", cost_test_reg)
...
print('mean square error : ', np.sqrt(metrics.mean_squared_error(Y_test, y_pred_test)))
print(accuracy_score(np.round(Y_test), np.round((pd.DataFrame(y_pred_test)))),'is the accuracy score of the linear regression' )#score
